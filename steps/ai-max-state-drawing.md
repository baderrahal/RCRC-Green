...
11. Three breaks watched red and reversed byte for byte.
...
swaps by kind and each kind's choice type is the one record. The section's report line names
only what a section really copies and the choice's sentence carries Crop View with whose
setting it is, so every created section's three settings are in the report the way a plan
view's are. The closing note, revit-commands.md, core-rules.md and CLAUDE.md all stop saying
a section copies all three, and CLAUDE.md counts three tool settings now. What this does not
fix is written down: the model's own sections still have their crops off and still draw, and
bounding those is the user's call. Merged as `b7d41dd` from pull request 79 with 1224 tests
on the runner, 0 failed and 0 skipped, the local count after the last file was written and
three above the 1221 main carried at `9fd1a65`. The merged main reads 1224 locally. The
squash message came back byte for byte. Two breaks watched red.
...
corner and a viewport by its centre, so a schedule is measured after placement and moved to
the centre it was asked for, and the views divide a DrawingArea rather than the whole sheet,
the title strip being the tool's own setting because a placed block reports its size and
nothing about where its strip begins. THE CROP IS NOT ANNOTATION CROP: DM-11-(010) Overall
Plan has it ON and two foreign sections still draw in it, both the model's own with Crop
View off and unbounded extents, and the tool computes a section box from the plot's scope
box and then copies Crop View off from the sibling four calls later. What is UNKNOWN there
and what would settle it in Revit are in the log, with the referencing sheet finding. Pull
request 76, fixes 5 to 8, merged as `148e44b` with 1221 tests on the runner, 0 failed and 0
skipped, which is this branch's 1216 plus the five KPI pull request 75 landed while it was
open: the report says a scale Revit labels Custom as well as its ratio, a new viewport takes
its type off the sibling instead of the document's default, the eleven title blocks are
remembered in two settings files read user first with Core merging them under test, and
every report goes to the repo folder only with nothing written when the pointer is missing.
That last one reaches the KPI pane through ReportFile and ReportPlaces and THE KPI SESSION
HAS TO BE TOLD. Main reads 1221 locally on `148e44b`. Four breaks watched red. Both squash
messages came back byte for byte. The mockup is design/pr-76.
...
namespace had to follow, so the suite reads 912 before and after. The rules are in
`.claude/rules/territory.md` and the wall is `.claude/hooks/territory-check.sh`, wired in
settings.json: two tasks in one commit refuse, Shared next to a task refuses, one task
plus common files passes, Shared plus common passes. Three refusals and two allowed
shapes were probed by hand against a scratch index and the refusals watched, and the
state file hook was probed both ways. This log and this state file are the split halves
of the old shared pair, drawing and kpi each with their own, pointers left behind, and
...
every one not read is listed in `steps/log-drawing.md`.
...
of the 53 stay written down in `steps/log-drawing.md` as reported and untouched. Four of the twelve
...
full item by item table is the oldest entry in `steps/log-drawing.md`.
...
Phase 8 review findings in `steps/log-drawing.md`. Nothing was fixed in response to them. The phase 8
run is closed.
...
`steps/log-drawing.md` entry lists what that leaves unchecked, item by item.
