...
namespace RcrcGreen.Core.Tests.Kpi
{
    ...
    public static class PdfFixture
    {
        ...
        /// </summary>
        public static string Form(string folder, string fileName, IEnumerable<KeyValuePair<string, string>> fields)
        {
        ...
        public static byte[] Bytes(IEnumerable<KeyValuePair<string, string>> fields)
        {
            var held = new List<KeyValuePair<string, string>>(fields);
            ...
            foreach (KeyValuePair<string, string> one in held)
            {
                references.Add(at + " 0 R");
                annotations.Append(at + " 0 R ");

                if (string.Equals(one.Key, ParentedFieldName, StringComparison.Ordinal))
                {
                    ...
                    objects.Add((at + 1) + " 0 obj\n<</Parent " + at + " 0 R/T(0)/V"
                        + Literal(one.Value) + "/Type/Annot/Subtype/Widget/Rect[0 0 10 10]/P 4 0 R"
                        + "/DA(/Helv 0 Tf 0 g)>>\nendobj\n");
                    at = at + 2;
                    continue;
                }
                ...
                objects.Add(at + " 0 obj\n<</Type/Annot/Subtype/Widget/FT/Tx/T" + Literal(one.Key)
                    + "/V" + Literal(one.Value) + appearance
                    + "/DA(/Helv 0 Tf 0 g)/Rect[0 0 10 10]/P 4 0 R>>\nendobj\n");
                at = at + 1;
            }
        ...
        public static KeyValuePair<string, string> Field(string name, string note)
        {
            return new KeyValuePair<string, string>(name, note);
        }
    }
}
